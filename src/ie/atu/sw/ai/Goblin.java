...
public class Goblin extends GameCharacterNN implements GameCharacterable {
	...
	// This is the neural network for the Goblin
	...
	public static void loadNeuralNetwork(boolean forceNNRebuild) throws Exception {
		...
		// Normalise the input data for the Goblin between the range 0 and 1
		Aicme4jUtils.normalise(data, 0, 1);

		nn = NetworkBuilderFactory.getInstance().newNetworkBuilder().inputLayer("Input", data[0].length)
				.hiddenLayer("Hidden", Activation.RELU, 6).outputLayer("Output", Activation.LINEAR, expected[0].length)
				.train(data, expected, 0.0001, 0.95, 1_000_00, 0.00001, Loss.MAE).save(NN_PATH).build();
	...
	public void fight(Weapon weapon, Player opponent) {
		...
		if (!this.isAlive())
			return; // Check if the Goblin is alive
