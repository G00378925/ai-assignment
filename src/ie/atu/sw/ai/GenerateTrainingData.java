...
public class GenerateTrainingData {
	...
	private static final String[] GC_NAMES = { "Goblin", "Imp", "Troll" };

	// This the function that the goblin will be learning.
	private static final BiFunction<Double, Double, Double> goblinAttack = (attack,
			defence) -> ((attack + defence) * 0.75);

	// Imp will be learning this one.
	private static final BiFunction<Double, Double, Double> impAttack = (attack, defence) -> Math
			.floor((attack + defence) / 50);
	...
	private static final BiFunction<Double, Boolean, Double> trollAttackPunch = (attack,
			sharp) -> (attack * (sharp ? 0.2 : 0.3));
	private static final BiFunction<Double, Boolean, Double> trollAttackKick = (attack, sharp) -> (attack * 0.3)
			* (sharp ? 1.25 : 1);
